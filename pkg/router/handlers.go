...
func (s *Server) handleAgentInvoke(c *gin.Context) {
	...
	c.Header("x-agentcube-session-id", newSessionID)

	// Forward request to sandbox
	s.forwardToSandbox(c, endpoint, path)
}
...
func (s *Server) forwardToSandbox(c *gin.Context, endpoint, path string) {
	...
	proxy.ModifyResponse = func(resp *http.Response) error {
		// Add session ID to response header if not already present
		if resp.Header.Get("x-agentcube-session-id") == "" {
			sessionID := c.GetHeader("x-agentcube-session-id")
			if sessionID != "" {
